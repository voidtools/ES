...
#define EVERYTHING_IPC_FILTER_IMAGE										6
...
#define EVERYTHING_IPC_RESTART											6 // returns 1 if the program restarts. (everything 1.5)
...
#define EVERYTHING_IPC_SAVE_DB_NOW										413 // same as EVERYTHING_IPC_SAVE_DB, except blocks until db is saved.
...
#define EVERYTHING_IPC_IS_FOLDER_SIDEBAR_SUBFOLDERS						519 // int is_subfolders = (int)SendMessage(FindWindow(EVERYTHING_IPC_SEARCH_CLIENT_WNDCLASS,0),EVERYTHING_WM_IPC,EVERYTHING_IPC_IS_FOLDER_SIDEBAR_SUBFOLDERS,0); 
...
typedef struct EVERYTHING_IPC_LIST2
{
	...
	// the number of items available in this structure.
	DWORD numitems;
	...
	// might be different to requested sort type.
	DWORD sort_type;
